...
export class MemoryGame {
    ...
    flippedIndices = [];
    activePlayer = 0;
    constructor(symbols, pairCount) {
        if (symbols.length < pairCount) {
            throw new Error('Nicht genug Symbole für die gewählte Spielfeldgröße.');
        }
        // Die ersten Symbole sind themenpassend; ein paar Reserven sorgen für Abwechslung.
        const candidates = symbols.slice(0, pairCount + SPARE_SYMBOLS);
        const pairSymbols = shuffle(candidates).slice(0, pairCount);
    ...
    resolveTurn() {
        ...
        const indices = [this.flippedIndices[0], this.flippedIndices[1]];
